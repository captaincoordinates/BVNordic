...
DOCKER_REPO=tomfumb
DOCKER_IMAGE=bvnordic-osm-exporter:1
...
cicd/scripts/pull_or_build.sh repo=$DOCKER_REPO image=$DOCKER_IMAGE build_dir=cicd/osm/docker/export context_dir=cicd/osm upload_if_missing=$UPLOAD_IF_MISSING
docker run --rm -w /data -v $PWD:/data $DOCKER_TAG ogr2ogr -sql "SELECT t.geom, t.dog_friend AS dog_friend, t.lights AS lights, t.difficulty AS difficulty, COALESCE(t.closed, 0) as closed, tn.trail_name as trail_name, CASE WHEN (t.segment_end_1 IS NOT NULL AND t.segment_end_2 IS NOT NULL) THEN COALESCE(t.segment_name, tn.trail_name) || ' ' || t.segment_end_1 || ' to ' || t.segment_end_2 ELSE COALESCE(t.segment_name, tn.trail_name) END AS segment_name FROM trails t LEFT JOIN trail_names tn ON t.trail_id = tn.trail_id" $TMP_JOINED main-data.gpkg
