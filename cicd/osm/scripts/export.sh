...
docker run --rm -w /data -v $PWD:/data $DOCKER_TAG ogr2ogr -sql "SELECT t.geom, t.dog_friend AS dog_friend, t.lights AS lights, t.difficulty AS difficulty, tn.trail_name AS name FROM trails t LEFT JOIN trail_names tn ON t.trail_id = tn.trail_id" $TMP_JOINED main-data.gpkg
