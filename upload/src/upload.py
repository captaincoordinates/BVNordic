...
from .settings import ROOT_FOLDER_NAME, MIME_FOLDER, LATEST_FOLDER_NAME, MIME_SHORTCUT
...
def get_or_create_folder(service, name: str) -> DataObject:
    ...
    folder_search: Final = service.files().list(q=f"name='{safe_name}' and mimeType='{MIME_FOLDER}'").execute().get("files")
...
def create_or_update_latest(service, name: str, file_path: str, parent_folder_id: str) -> None:
    safe_name: Final = re.sub('\'', '_', name)
    ...
    if len(search) == 0:
        ...
        remote_file_id = service.files().create(
            body={
                "name": safe_name,
...
def upload_folder(service, folder_name: str, root_path: str, update_latest: bool) -> None:
    ...
    for root, _, files in walk(upload_path):
        ...
        for file in files:
            LOGGER.info(f"uploading file {path.basename(file)} within {path.basename(root)}")
            remote_file_id = service.files().create(
                body={
            ...
            if update_latest and local_unique_path in latests:
                ...
                create_or_update_latest(service, f"latest-{file}", path.join(root, file), remote_root_folder.id)
...
if __name__ == "__main__":
    ...
    parser.add_argument("upload_folder_name", type=str, help="Name of the folder to be uploaded")
    parser.add_argument("upload_folder_root_path", type=str, help="Path for the parent of the folder to be uploaded")
    parser.add_argument("update_latest", type=bool, help="If this upload should be considered the latest content", default=False)
    args = vars(parser.parse_args())

    configure_logging()
    upload_folder(create_service(), args["upload_folder_name"], args["upload_folder_root_path"], args["update_latest"])
